...
const Module = () => {
  const { seconds, minutes, moves, handleRestart } = useContext(GlobalContext)

  return (
    <section className='w-screen h-screen absolute top-0 left-0 flex justify-center items-center bg-[rgba(0,0,0,0.5)]'>
      <article
        className='bg-lightText rounded-lg p-12 w-[600px] h-[600px] flex flex-col items-center justify-center gap-4 
      max-[768px]:w-full max-[768px]:p-4 mx-4'
      >
        <h2 className='text-5xl font-semibold text-buttonDark max-[768px]:text-4xl'>
          You did it!
        </h2>
